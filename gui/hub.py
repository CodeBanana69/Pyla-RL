...
from utils import (
    LOCAL_BRAWL_STARS_API_CONFIG_PATH,
...
class Hub:
    ...
    def __init__(self,
        ...
        self.brawl_stars_api_base_config_path = "cfg/brawl_stars_api.toml"
        self.brawl_stars_api_config_path = LOCAL_BRAWL_STARS_API_CONFIG_PATH
        ...
        self.brawl_stars_api_config = dict(load_toml_as_dict(self.brawl_stars_api_base_config_path))
        if Path(resolve_project_path(self.brawl_stars_api_config_path)).exists():
            self.brawl_stars_api_config.update(load_toml_as_dict(self.brawl_stars_api_config_path))
        ...
        self.brawl_stars_api_config.setdefault(
            "key_description",
            "Auto-generated by PylaAi-XXZ for the current public IP.",
        )
    ...
    def _init_api_tab(self):
        ...
        tutorial_text = (
            "Use this for Push All trophy auto-fill:\n"
            "1. Copy your player tag from your Brawl Stars profile and paste it below.\n"
            "2. Recommended: enable Auto Refresh Token and enter your Brawl Stars developer login.\n"
            "3. PylaAi-XXZ will create an API key for your current public IP when Push All needs it.\n"
            "4. If you use a VPN, connect it before testing or starting Push All.\n\n"
            "Manual token mode:\n"
            "1. Create a key at developer.brawlstars.com for your current public IP.\n"
            "2. Paste that key into API Token and turn Auto Refresh Token off.\n\n"
            "Saved safely: the GUI writes these values to cfg/brawl_stars_api.local.toml, which is ignored by Git."
        )
        tutorial = ctk.CTkLabel(
            container,
            text=tutorial_text,
        ...
        def open_brawl_stars_developer():
        ...
        developer_btn.grid(row=row_idx, column=0, columnspan=2, sticky="w", padx=S(20), pady=(0, S(12)))
        self.attach_tooltip(developer_btn, "Opens the official Brawl Stars developer portal.")
        row_idx += 1

        def create_api_entry(label_text, config_key, convert_func=str, width=360, show=None):
            nonlocal row_idx
            lbl = ctk.CTkLabel(container, text=label_text, font=("Arial", S(18)))
            lbl.grid(row=row_idx, column=0, sticky="e", padx=S(20), pady=S(10))
            var_str = tk.StringVar(value=str(self.brawl_stars_api_config.get(config_key, "")))
            entry_vars[config_key] = (var_str, convert_func)

            def on_save(*_):
                val_str = var_str.get().strip()
                try:
                    self.brawl_stars_api_config[config_key] = convert_func(val_str)
                    save_api_config()
                except ValueError:
                    var_str.set(str(self.brawl_stars_api_config.get(config_key, "")))
        ...
        def create_api_toggle(label_text, config_key):
            nonlocal row_idx
            lbl = ctk.CTkLabel(container, text=label_text, font=("Arial", S(18)))
            lbl.grid(row=row_idx, column=0, sticky="e", padx=S(20), pady=S(10))
            var_bool = tk.BooleanVar(value=bool(self.brawl_stars_api_config.get(config_key, False)))

            def on_toggle():
                self.brawl_stars_api_config[config_key] = bool(var_bool.get())
                save_api_config()
        ...
        def sync_api_entries():
            for config_key, (var_str, convert_func) in entry_vars.items():
                self.brawl_stars_api_config[config_key] = convert_func(var_str.get().strip())
            save_api_config()

        create_api_entry("Player Tag:", "player_tag", str, width=220)
        create_api_toggle("Auto Refresh Token:", "auto_refresh_token")
        create_api_entry("Developer Email:", "developer_email", str, width=360)
        ...
        test_btn.grid(row=row_idx, column=0, columnspan=2, padx=S(20), pady=S(12))
        self.attach_tooltip(test_btn, "Checks the saved API settings and loads your Brawl Stars profile.")
        row_idx += 1
