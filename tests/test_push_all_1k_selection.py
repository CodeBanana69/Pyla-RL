...
class PushAll1kSelectionTest(unittest.TestCase):
    ...
    def test_push_all_priority_order_moves_selected_brawlers_first(self):
        ...
        data = [
            {"brawler": "shelly", "automatically_pick": False, "selection_method": "highest_trophies"},
            {"brawler": "max", "automatically_pick": True, "selection_method": "highest_trophies"},
            {"brawler": "spike", "automatically_pick": True, "selection_method": "highest_trophies"},
        ]
        ...
        self.assertEqual(ordered[2]["selection_method"], "highest_trophies")
    ...
    def test_push_all_build_queue_sorts_highest_first(self):
        obj = object.__new__(SelectBrawler)
        ...
        with patch("gui.brawler_queue.load_brawl_stars_api_config", return_value={
            "api_token": "token",
            "player_tag": "TAG",
            "timeout_seconds": 15,
        }), patch("gui.brawler_queue.fetch_brawl_stars_player", return_value=player_data):
            data = SelectBrawler.get_push_all_data(obj, 1000)

        self.assertEqual([row["brawler"] for row in data], ["meg", "colt", "shelly"])
        self.assertEqual(data[0]["trophies"], 750)
        self.assertEqual(data[0]["selection_method"], "highest_trophies")
    ...
    def test_push_all_targets_all_use_highest_trophies_selection_method(self):
        obj = object.__new__(SelectBrawler)
        obj.brawlers = ["shelly", "colt", "meg"]
        player_data = {
            "brawlers": [
        ...
        with patch("gui.brawler_queue.load_brawl_stars_api_config", return_value={
            ...
            for target in (250, 500, 750, 1000, 1250, 1500):
                with self.subTest(target=target):
                    ...
                    self.assertTrue(all(row["selection_method"] == "highest_trophies" for row in data))
