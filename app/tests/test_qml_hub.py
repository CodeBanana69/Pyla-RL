...
class QmlHubStateTests(unittest.TestCase):
    ...
    def test_preflight_actions_preserve_cache_and_stay_async(self):
        ...
        self.assertIn("def _invalidate_preflight_cache(self):", bridge)
        self.assertIn("def _has_fresh_ready_preflight(self, max_age_seconds=300):", bridge)
        self.assertIn("if not self._has_fresh_ready_preflight():", bridge)
        self.assertIn("self._invalidate_preflight_cache()", bridge)
        self.assertIn("self._action_worker = worker", bridge)
