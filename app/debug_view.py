...
class DebugViewPublisher:
    ...
    def from_config(cls):
        ...
        enabled = config_bool(debug_config.get("debug_view"), False) or config_bool(
            general_config.get("visual_debug"), False
        )
        max_fps = general_config.get("visual_debug_max_fps") or debug_config.get(
            "debug_view_fps", DEFAULT_DEBUG_VIEW_FPS
        )
        advanced_visuals = False
        if enabled:
            advanced_visuals = config_bool(debug_config.get("advanced_debug_visuals"), False) or config_bool(
                general_config.get("advanced_visuals"), False
            )
        record_clips = enabled and config_bool(debug_config.get("record_debug_preview_clips"), False)
        return cls(
            enabled=enabled,
            max_fps=max_fps,
            title=DEBUG_VIEW_TITLE,
    ...
    def start_viewer(self, frame):
        ...
        if self.process and self.process.poll() is not None:
            ...
            self.close()
            if ran_for < 2.0:
                log_hint = " See logs/debug_view_worker.log for details." if _running_as_compiled_executable() else ""
        ...
        if _running_as_compiled_executable():
            log_path = resolve_project_path("logs/debug_view_worker.log")
            os.makedirs(os.path.dirname(log_path), exist_ok=True)
            self.worker_log_file = open(log_path, "a", encoding="utf-8")
...
def draw_range_circles(image, player_boxes, attack_range, super_range, effective_enemy_range=0):
    if not player_boxes:
        return
    ...
    try:
        ...
        spacing_radius = int(float(effective_enemy_range or 0))
    except (TypeError, ValueError):
        return

    if attack_radius <= 0 and super_radius <= 0 and spacing_radius <= 0:
        return

    x1, y1, x2, y2 = [int(v) for v in player_box[:4]]
    center = ((x1 + x2) // 2, (y1 + y2) // 2)
    if spacing_radius > 0:
        cv2.circle(image, center, spacing_radius, (160, 32, 240), 5)
    elif attack_radius > 0 and super_radius > 0 and attack_radius == super_radius:
...
def draw_close_tile_debug(image, close_tile_debug):
    if not close_tile_debug:
        return

    crop = close_tile_debug.get("crop")
    if crop and len(crop) >= 4:
        x1, y1, x2, y2 = [int(v) for v in crop[:4]]
...
class DebugClipRecorder:
    ...
    def start(self, now):
        clip_dir = resolve_project_path("debug_frames/clips")
...
def draw_debug_data(image, debug_data, width, height):
    ...
    draw_range_circles(
        image,
        player_boxes,
        debug_data.get("attack_range"),
        debug_data.get("super_range"),
        debug_data.get("effective_enemy_range"),
    )
    draw_poison_gas_lines(image, player_boxes, debug_data.get("poison_gas"))
    draw_close_tile_debug(image, debug_data.get("close_tile_debug"))
    ...
    state = debug_data.get("status") or debug_data.get("state")
    if state:
    ...
    intent = debug_data.get("match_intent")
    if intent:
        cv2.putText(
            image,
            str(intent)[:96],
