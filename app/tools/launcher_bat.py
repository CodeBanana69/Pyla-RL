...
if exist "app\\.venv\\Scripts\\python.exe" (
    "app\\.venv\\Scripts\\python.exe" -c "{_RUNTIME_IMPORT_CHECK}" >nul 2>&1
    if not errorlevel 1 (
...
def candidate_python_commands() -> list[tuple[str, list[str]]]:
    ...
    pin = bundle_dir / "cfg" / "pyla_python.txt"
    if pin.exists():
...
def create_run_file(
    ...
    del python_command
    install_dir = Path(project_dir)
    if python_executable:
        from tools.python_runtime import write_python_pin

        write_python_pin(install_dir / "app", python_executable)

    remove_legacy_launchers(install_dir)

    run_bat = install_dir / RUN_BAT_NAME
